...
class PerformanceEvaluator:
    ...
    def evaluate_single_alpha(self, alpha_name):
        try:
            ...
            returns = self.alpha_performance_evaluating_utils.data_loader.load_processed_window_list("pv_1min_return",
                                                                                                     trade_dates)
            returns["code"] = returns["code"].apply(lambda x: x.decode('utf-8'))
            returns["date"] = returns["date"].astype(int)

            # adjust limit
            limit = returns.pivot_table(index="date", columns="code", values="return").sort_index()
            limit = limit.applymap(lambda x: abs(x) > 0.095)
            print(limit.sum().describe())
